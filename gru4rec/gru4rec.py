...
class GRU4Rec:
    ...
    def __init__(self, loss='bpr-max', final_act='linear', hidden_act='tanh', layers=[100],
                 ...
                 sigma=0.0, init_as_normal=False, train_random_order=False, time_sort=True,
                 session_key='SessionId', item_key='ItemId', time_key='Time',
                 preset_embeddings=None, embedding_transform_matrix=False):
        self.layers = layers
        ...
        self.embedding_transform_matrix = embedding_transform_matrix
    ...
    def init(self, data, reset=True):
        ...
        elif self.preset_embeddings is not None:
            max_item = np.max(np.array(self.itemidmap.index))
            preset_embeddings_recoded = np.zeros((max_item+1, self.preset_embeddings.shape[1]))
            preset_embeddings_recoded[self.itemidmap.values] = self.preset_embeddings[self.itemidmap.index]
            self.E = theano.shared(self.floatX(preset_embeddings_recoded), borrow=True)
            n_features = self.preset_embeddings.shape[1]
            if self.embedding_transform_matrix:
                self.Ex = self.init_weights((n_features, n_features), name='Ex')
        else:
...
#            updates[acc] = T.set_subtensor(acc_s, acc_new) #Faster, but inaccurate when an index occurs multiple times
    ...
    def model(self, X, H, M, R=None, Y=None, drop_p_hidden=0.0, drop_p_embed=0.0, predict=False):
        ...
        if self.constrained_embedding:
            ...
            Sx = self.E[X]
            y_ = self.dropout(Sx, drop_p_embed)
            if self.embedding_transform_matrix:
                y = T.dot(y_, self.Ex)
            else:
                y = y_
    ...
    def generate_neg_samples(self, pop, length):
        ...
        return sample
    def fit(self, data, reset=True, sample_store=10000000):
        '''
        Trains the network.
        ...
        if(reset):
            ...
            self.idmapdf = pd.DataFrame({self.item_key:itemids, 'ItemIdx':self.itemidmap[itemids].values})
        data = pd.merge(data, self.idmapdf, on=self.item_key, how='inner')
        offset_sessions = self.init(data, reset=reset)
        if self.n_sample:
